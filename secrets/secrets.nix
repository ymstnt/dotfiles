let
  keys = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIG0MpAJw5Q5t4yaxX88Xr1cUjMHEfjcfq++v7efoqTIa root@cassiopeia"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAHfr8D6mA/MhI/5CQF/NiVCBiwWheLog31vpFmXtIu8 root@andromeda"

    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDVor+g/31/XFIzuZYQrNK/RIbU1iDaSyOfM8re73eAd ymstnt@cassiopeia"
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEVxinYyV/gDhWNeSa0LD6kRKwTWhFxXVS23axGO/2sa ymstnt@andromeda"
  ];
in
{
  "calcardav-url.age".publicKeys = keys;
  "calcardav-username.age".publicKeys = keys;
  "calcardav-password.age".publicKeys = keys;
}
